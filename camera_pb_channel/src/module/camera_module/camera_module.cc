...
bool CameraModule::Initialize(aimrt::CoreRef core) {
  core_ = core;
  ...
  try {
    ...
    bool ret = aimrt::channel::RegisterPublishType<aimrt_samples::protocols::Image>(publisher_);
...
bool CameraModule::Start() {
  try {
    ...
    executor_.Execute(std::bind(&CameraModule::MainLoop, this));
  } catch (const std::exception& e) {
...
void CameraModule::MainLoop() {
  try {
    ...
    while (run_flag_) {
      ...
      // full pb event
      aimrt_samples::protocols::Image img;

      AIMRT_INFO("Publish new pb event, data: {}", aimrt::Pb2CompactJson(img));

      // publish pb event
      aimrt::channel::Publish(publisher_, img);
    }
